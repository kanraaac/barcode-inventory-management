...
function handleStockOut() {
  ...
  for (var i = 2; i <= lastRow; i++) {
    ...
    // 출고 이력 시트에 입력
    sheetHistory.insertRowsBefore(3, 1);
...
function handleStockUse() {
  ...
  for (var i = 2; i <= lastRow; i++) {
    ...
    sheetHistory.getRange(3, 3, 1, stockRow.length).setValues([stockRow]);
    // 노란색 배경 적용
    sheetHistory.getRange(3, 1, 1, stockRow.length + 2).setBackground('#FFFF00');
    ...
    sheetRecord.getRange(4, 3, 1, stockRow.length + 1).setValues([[barcode].concat(stockRow)]);
    // 노란색 배경 적용
    sheetRecord.getRange(4, 1, 1, stockRow.length + 2).setBackground('#FFFF00');
    // 현재고 정보에서 삭제
    sheetStock.deleteRow(foundRow);
